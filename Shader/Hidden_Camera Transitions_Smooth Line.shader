...
void main()
{
    u_xlat0.x = abs(_Angle.y) + abs(_Angle.x);
    u_xlat0.xy = _Angle.xy / u_xlat0.xx;
    u_xlat4 = dot(u_xlat0.xy, vec2(0.5, 0.5));
    u_xlat0.x = dot(u_xlat0.xy, vs_TEXCOORD0.xy);
    u_xlat2.x = u_xlat4 + -0.5;
    u_xlat4 = _Smoothness + 1.0;
    u_xlat2.x = _T * u_xlat4 + u_xlat2.x;
