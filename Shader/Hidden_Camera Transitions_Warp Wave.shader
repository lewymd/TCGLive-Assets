...
#endif
uniform 	float _T;
uniform 	float _WarpWaveCurvature;
...
void main()
{
    u_xlat0.x = _T * 2.0 + vs_TEXCOORD0.y;
    ...
    u_xlat2.xz = vs_TEXCOORD0.xy + (-vec2(_WarpWaveCurvature));
    u_xlat1.xy = u_xlat2.xz * vec2(u_xlat4) + vec2(_WarpWaveCurvature);
    u_xlat0.xy = u_xlat2.xz * u_xlat0.xx + vec2(_WarpWaveCurvature);
    u_xlat16_0.xyz = texture(_MainTex, u_xlat0.xy).xyz;
    u_xlat16_1.xyz = texture(_SecondTex, u_xlat1.xy).xyz;
    u_xlat1.xyz = (-u_xlat16_0.xyz) + u_xlat16_1.xyz;
    SV_Target0.xyz = vec3(_T) * u_xlat1.xyz + u_xlat16_0.xyz;
    SV_Target0.w = 1.0;
    return;
}
...
void main()
{
    u_xlat0.x = _T * 2.0 + vs_TEXCOORD0.x;
    u_xlat0.x = u_xlat0.x + -1.0;
#ifdef UNITY_ADRENO_ES3
    u_xlat0.x = min(max(u_xlat0.x, 0.0), 1.0);
#else
...
void main()
{
    ...
    u_xlat1.xy = u_xlat2.xz * vec2(u_xlat4) + vec2(_WarpWaveCurvature);
    u_xlat0.xy = u_xlat2.xz * u_xlat0.xx + vec2(_WarpWaveCurvature);
    u_xlat16_0.xyz = texture(_SecondTex, u_xlat0.xy).xyz;
